...
Agent = (function() {
  ...
  function Agent() {
    ...
    this.socket.on('tc_CHANGE_NOTE', function(payload) {
      return noteManager.changeNote(payload.nid, payload.content);
    });
    this.socket.on('tc_MOVE_NOTE', function(payload) {
      return noteManager.moveNote(payload.nid, payload.xr, payload.yr);
    });
    this.socket.on('tc_DELETE_NOTE', function(nid) {
      return noteManager.deleteNote(nid);
    });
  }
